...
class ServiceMenu extends StatefulWidget {
  ...
  const ServiceMenu({Key key, this.username,this.mobileNumber}) : super(key: key);
  @override
...
class _ServiceMenuState extends State<ServiceMenu> {
  ...
  void fetchData() async {
    var response = await networkHandler.get(getservices+"${widget.username}");
    setState(() {
  ...
  void fetchTimings(d) async {
    var response = await networkHandler.get(
        timings+"${widget.username}/${DateFormat('y-M-d').format(date.add(Duration(days: d))).toString()}");
        ...
        body: TabBarView(
          children: [
            listModel.data == null || services == false
                ? Container(
                  child: Center(
                      child: services ? page : Text("Not providing any services yet"),),
                )
                : Stack(
                  children: [
                    Container(
