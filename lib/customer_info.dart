...
import 'package:customer1/Models/OrderModel.dart';

import 'package:customer1/Models/ServiceAppointment.dart';
import 'package:customer1/Models/ServiceModel.dart';
import 'package:customer1/Models/SlotsModel.dart';
import 'package:customer1/Secret.dart';
...
class CustomerInfo extends StatefulWidget {
  ...
  List<ServiceModel> cart =<ServiceModel>[];
  String totalCost;
  int totalHr;
  int totalMin;
  String mobileNumber;

  CustomerInfo({Key key, this.username,this.cart,this.totalCost,this.totalHr,this.totalMin,this.mobileNumber}) : super(key: key);
                  ...
                  if(response['Status']){
                    print("status");
                          ...
                          var response = await networkHandler.patch1(
                              addSlots+"${widget.username}/${DateFormat('y-M-d').format(date.add(Duration(days: y))).toString()}",
                              slots.toJson());
                          print("slots updated");
                          if (response.statusCode == 200 ||
                            ...
                            order.customer_contact_number = xyz.identifier;
                            order.Total = widget.totalCost;
                            order.total_time = "${add.hour}:${add.minute}";
                            ...
                            if (response.statusCode == 200 ||
                              ...
                              Appointment appointment = Appointment(
                                businessName: widget.username,
                                customerName: order.customer_name,
                                mobileNumber: widget.mobileNumber,
                                appointmentDate: DateFormat('MMM d')
                                    .format(date.add(Duration(days: y)))
                                    .toString(),
                                appointmentTime: order.start_time,
                                totalTime: order.total_time,
                                total: order.Total,
                                services: order.cart.length.toString(),
                              );
